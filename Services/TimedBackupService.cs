// File: Services/TimedBackupService.cs
//
// PURPOSE: Runs an automatic backup on a timer (default: every 5 minutes).
// This is the "safety net" that ensures backups happen even if no write-
// triggered backups fire. During the festival, this means at most 5 minutes
// of data can be lost if the container is destroyed unexpectedly.
//
// HOW IT WORKS:
//   - Inherits from BackgroundService, which is ASP.NET Core's built-in
//     base class for long-running background tasks.
//   - ASP.NET Core starts this automatically when the app boots
//     (because it's registered with AddHostedService<T>()).
//   - Routes all backups through BackupQueueService to respect the
//     semaphore — if a write-triggered backup is already running, the
//     timed backup silently skips that cycle.
//
// LIFETIME: Registered as a Hosted Service (Singleton lifetime, managed
// by the ASP.NET Core host). Starts when the app starts, stops when
// the app shuts down.
